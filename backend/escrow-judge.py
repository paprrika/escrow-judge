...
class _NativeRecipient:
    """Minimal chain-layer interface used for native GEN transfers to EOAs."""
...
class EscrowArbiter(gl.Contract):
    ...
    def __init__(self):
        ...
        self.seller_paid = u256(0)
        self.buyer_refunded = u256(0)
        self.bonds_refunded = u256(0)
        for p in PHASES_ALL:
    ...
    def _require_dispute_for_deal(self, deal_id: str, dispute_id: str) -> Dispute:
        if dispute_id not in self.disputes:
            _arb_err(f"unknown dispute {dispute_id}")
        dp = self.disputes[dispute_id]
        if dp.deal_id != deal_id:
            _arb_err(f"dispute {dispute_id} is not bound to deal {deal_id}")
        return dp

    def _deal_remaining(self, d: Deal) -> int:
        remaining = int(d.funded) - int(d.released) - int(d.refunded)
        if remaining < 0:
            _escrow_err(f"deal {d.deal_id} settlement exceeds funding")
        return remaining

    def _settlement_tranche_value(self, deal_id: str, idx: int) -> int:
        """Return a tranche value, assigning division dust to the last tranche."""
        d = self.deals[deal_id]
        lst = self.milestones[deal_id]
        target = None
        total_bps = 0
        every_other_released = True
        for i in range(len(lst)):
            m = lst[i]
            total_bps += int(m.share_bps)
            if int(m.idx) == idx:
                target = m
            elif not m.released:
                every_other_released = False
        if target is None:
            _escrow_err(f"milestone idx {idx} not found")
        if total_bps == 10000 and every_other_released:
            return self._deal_remaining(d)
        return (int(d.total) * int(target.share_bps)) // 10000

    def _send_value(self, recipient: Address, amount: int) -> None:
        if amount > 0:
            _NativeRecipient(recipient).emit_transfer(value=u256(amount))

    def _record_escrow_settlement(
        self,
        deal_id: str,
        seller_amount: int,
        buyer_amount: int,
        ...
        if seller_amount < 0 or buyer_amount < 0:
            _escrow_err("negative settlement amount")
        settled = seller_amount + buyer_amount
        d = self.deals[deal_id]
        remaining = self._deal_remaining(d)
        if settled > remaining:
            _escrow_err(
                f"settlement {settled} exceeds deal {deal_id} remaining {remaining}"
            )
        if settled > int(self.escrow_locked):
            _escrow_err("global escrow liability underflow")
        d.released = u256(int(d.released) + seller_amount)
        d.refunded = u256(int(d.refunded) + buyer_amount)
        self.deals[deal_id] = d
        self.escrow_locked = u256(int(self.escrow_locked) - settled)
        self.seller_paid = u256(int(self.seller_paid) + seller_amount)
        self.buyer_refunded = u256(int(self.buyer_refunded) + buyer_amount)

    def _refund_dispute_bond(self, dp: Dispute) -> int:
        if dp.bond_refunded:
            return 0
        amount = int(dp.bond)
        if amount > int(self.bonds_locked):
            _escrow_err("global bond liability underflow")
        dp.bond_refunded = True
        self.disputes[dp.dispute_id] = dp
        self.bonds_locked = u256(int(self.bonds_locked) - amount)
        self.bonds_refunded = u256(int(self.bonds_refunded) + amount)
        self._send_value(dp.opener, amount)
        return amount
    ...
    def open_deal(
        ...
        if int(total) <= 0:
            _escrow_err("total must be positive")
        if funded != int(total):
            _escrow_err(f"funding must equal total: funded {funded}, total {int(total)}")
        ...
        self.escrow_locked = u256(int(self.escrow_locked) + funded)
    ...
    def declare_milestone(
        ...
        for i in range(len(lst)):
            declared_bps += int(lst[i].share_bps)
            if int(lst[i].idx) == int(idx):
                _escrow_err(f"milestone idx {int(idx)} already declared")
        if declared_bps + int(share_bps) > 10000:
            _escrow_err("declared milestone shares exceed 10000 bps")
        m = Milestone(
            deal_id=deal_id,
            idx=u8(int(idx)),
    ...
    def release_milestone(self, deal_id: str, idx: u8) -> int:
        ...
        d = self.deals[deal_id]
        tranche = self._settlement_tranche_value(deal_id, int(idx))
        self._record_escrow_settlement(deal_id, tranche, 0)
        self._send_value(d.seller, tranche)
        m.released = True
        m.seller_paid = u256(tranche)
        self._replace_milestone(deal_id, m)
    ...
    def open_dispute(self, deal_id: str, idx: u8, ground: str, evidence_uri: str) -> str:
        ...
        dp = Dispute(
            ...
            bond_refunded=False,
            settled_seller=u256(0),
            settled_buyer=u256(0),
        )
        self.disputes[dispute_id] = dp
        self.bonds_locked = u256(int(self.bonds_locked) + int(gl.message.value))
    ...
    def file_arbiter_finding(self, deal_id: str, dispute_id: str, lean: str, notes: str) -> str:
        ...
        self._require_dispute_for_deal(deal_id, dispute_id)
    ...
    def finalize_dispute(self, deal_id: str, dispute_id: str) -> str:
        self._require_phase(deal_id, (PHASE_ARBITRATING,))
        dp = self._require_dispute_for_deal(deal_id, dispute_id)
        ...
        d = self.deals[deal_id]
        tranche = self._settlement_tranche_value(deal_id, int(dp.milestone_idx))
        seller_share = (tranche * int(dp.final_split_bps)) // 10000
        buyer_share = tranche - seller_share
        self._record_escrow_settlement(deal_id, seller_share, buyer_share)
        self._send_value(d.seller, seller_share)
        self._send_value(d.buyer, buyer_share)
        dp.settled_seller = u256(seller_share)
        dp.settled_buyer = u256(buyer_share)
        self.disputes[dispute_id] = dp
        self._refund_dispute_bond(dp)
        m.released = True
        m.seller_paid = u256(seller_share)
        m.buyer_refunded = u256(buyer_share)
        self._replace_milestone(deal_id, m)
        if self._all_released(deal_id):
            self._set_phase(deal_id, PHASE_CLOSED)
        else:
    ...
    def void_deal(self, deal_id: str, reason: str) -> int:
        ...
        remaining = self._deal_remaining(d)
        self._record_escrow_settlement(deal_id, 0, remaining)
        self._send_value(d.buyer, remaining)
        if deal_id in self.disputes_by_deal:
            dispute_ids = self.disputes_by_deal[deal_id]
            for i in range(len(dispute_ids)):
                did = dispute_ids[i]
                if did in self.disputes:
                    self._refund_dispute_bond(self.disputes[did])
    ...
    def deal(self, deal_id: str) -> dict:
        ...
        return {
            ...
            "locked": self._deal_remaining(d),
            "conserved": (
                int(d.funded)
                == int(d.released) + int(d.refunded) + self._deal_remaining(d)
            ),
            "phase": d.phase,
    ...
    def milestones_of(self, deal_id: str) -> list:
        ...
        for i in range(len(lst)):
            ...
            out.append({
                ...
                "seller_paid": int(m.seller_paid),
                "buyer_refunded": int(m.buyer_refunded),
            })
        return out
    ...
    def dispute(self, dispute_id: str) -> dict:
        ...
        return {
            ...
            "bond": int(dp.bond),
            "bond_refunded": bool(dp.bond_refunded),
            "settled_seller": int(dp.settled_seller),
            "settled_buyer": int(dp.settled_buyer),
        }
    ...
    def tranche_state(self, deal_id: str, idx: u32) -> dict:
        ...
        d = self.deals[deal_id]
        tranche = (
            int(m.seller_paid) + int(m.buyer_refunded)
            if m.released
            else self._settlement_tranche_value(deal_id, int(m.idx))
        )
        return {
            "deal_id": deal_id,
            "idx": int(m.idx),
    ...
    def platform_summary(self) -> dict:
        ...
        return {
            ...
            "bonds_locked": int(self.bonds_locked),
            "liabilities": int(self.escrow_locked) + int(self.bonds_locked),
            "liabilities_covered": (
                int(self.balance) >= int(self.escrow_locked) + int(self.bonds_locked)
            ),
            "seller_paid": int(self.seller_paid),
            "buyer_refunded": int(self.buyer_refunded),
            "bonds_refunded": int(self.bonds_refunded),
        }
