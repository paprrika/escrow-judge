...
export function DealCard({ deal, selected, myRole, onClick }: Props) {
  const settled = Number(deal.released) + Number(deal.refunded);
  const pct =
    Number(deal.total) > 0
      ? Math.min(100, Math.round((settled / Number(deal.total)) * 100))
      : 0;
  return (
    <motion.button
      layout
    ...
    >
      ...
      <div className={s.meta}>
        <span className="mono">
          {weiToGen(settled)} / {weiToGen(deal.total)} GEN settled
        </span>
        {myRole && myRole !== "NONE" && (
